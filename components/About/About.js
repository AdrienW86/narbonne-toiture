  ...
  return (
    <section className={styles.container}>
        <h1 className={styles.title}> Qui sommes-nous ? </h1>
            <p className={styles.intro}> ORTIZ Rénovation, c’est une équipe d’artisans passionnés, entièrement dédiés à vos projets de couverture et de rénovation. Fondée sur des valeurs fortes telles que la fiabilité, la qualité et l’engagement, notre entreprise est aujourd'hui reconnue pour son sérieux dans le domaine de la toiture. </p>

        <h2 className={styles.h2}> Une entreprise à taille humaine, proche de vos attentes </h2>
            <p className={styles.p}> Chez ORTIZ Rénovation, chaque projet est unique. Nous privilégions un accompagnement sur mesure, basé sur l'écoute attentive de vos besoins. Que vous soyez un particulier ou un professionnel, notre objectif est de vous apporter des solutions durables, esthétiques et adaptées à votre budget. </p>
        ...
        <p className={styles.intro}>ORTIZ Rénovation, c’est bien plus qu’une entreprise de couverture : c’est votre partenaire de confiance pour des travaux réalisés en toute tranquillité.</p>
    </section>
  )
}